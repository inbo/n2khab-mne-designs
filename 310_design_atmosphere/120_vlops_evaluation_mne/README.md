...
1. Code goes under `120_vlops_evaluation_mne` and can be versioned with git.
    ...
    - Other (larger) generated ('processed') data go under `data/20_processed` and are to be git-ignored.
...
Data source names and the associated R objects are as much as possible internationalized: English names for folder & files and for variable names of the R-object.
...
**Raw** data sources go in the first one, **processed** data sources in the second.
    ...
    They are mostly either _distributed_ (see below) raw data sources, or _non-distributed_, locally reproducible processed data sources.
        ...
        - In a few cases also processed data sources are worth distributing (when processing steps take very long): see below for the place where code is to be maintained in that case.
    - generated **text** data sources (not too large) can be versioned by git within the current repo and can be stored in `data/20_processed`.
...
Current setup is still temporary but to be followed for now:
    ...
    - so, add NEW _to-be-distributed_ data sources under the dedicated Q-folder.
...
As discussed, and you can read about it in a [powerpoint presentation](https://drive.google.com/open?id=1abWt-L3MAF0NeHEQgyi-e6hShiceX2pL) given to VMM on 12/03/2019, somewhere from the middle of the presentation and further.
...
- [VLOPS results](https://drive.google.com/open?id=17Kvgx1SCGWoUAORnnVbkx8gLSaxgJfkQ) of concentrations, dry deposition, wet deposition, total deposition at the 1 km<sup>2</sup> level, both calibrated and uncalibrated for years 1990 + 2000-2017 (however for 2017 the emission data of 2016 were used, only meteo is from 2017)
...
Interpretation (naming) of the different typegroups is currently in [this     gsheet](https://docs.google.com/spreadsheets/d/1n2ohvuLEK_anX37gxlanQXRGotweMVonFxKxtfTcuZI/edit#gid=556298356).
...
It has been used by VMM to produce the uncalibrated VLOPS point calculations for 2016 (see above).
...
Comprises reports, articles and some accompanying mail text.
