...
ssize_plotpath <- file.path(projroot, "plots/2025-09 sample_size_changes/")
if (!dir.exists(ssize_plotpath)) dir.create(ssize_plotpath, recursive = TRUE)
```

**SCENARIO: `r scenario_name`**

- sample size truncation:
  - GW schemes: truncate a type's domain-specific target sample size for at 20% of the population size in that domain
  - SOIL schemes: truncate a type's domain-specific sample size for at 1/3 of the population size in that domain
- reduce size of densification submodules:
  - drop 7140_oli and 2190_mp from the densification submodules
- reduce the _resulting_ target sample size of specific types by 50%, depending on the scheme
  - GW: 1310_pol, 1330_hpr, 2190_overig, 6430_mr, 6430_hw, 6510_hua, 6510_hus, 7230
  - SOIL: 1310_pol, 6510_hua, 6510_hus, 7230
- for types with sample size 0 or 1 for panel set 1 in included domains, increase to sample size 2 (as done at stratum level for Flanders, before)
...
targetcal_ref <- get("scheme_moco_ps_spsubset_targetfag_stratum_sppost_spsamples_calendar", envir = ref)
targetcal_new <- get(
...
Number of changed sample sizes in above table.
...
Number of changed sample sizes in above table, depending on whether sample was truncated using a maximum population proportion, or not.
...
#| fig.height: 10
#| fig.width: 12
flanders_ssizes <- plot_abs_sample_sizes(ssizes_new, flanders = TRUE)
flanders_ssizes
ggsave(
  file.path(
    ssize_plotpath,
    str_c("flanders_ssizes_", scenario_name, ".png")
  ),
  flanders_ssizes,
  width = 24,
  height = 13.5
...
#| fig.height: 10
#| fig.width: 12
sacs_ssizes <- plot_abs_sample_sizes(ssizes_new, flanders = FALSE)
sacs_ssizes
ggsave(
  file.path(
    ssize_plotpath,
    str_c("sacs_ssizes_", scenario_name, ".png")
  ),
  sacs_ssizes,
  width = 24,
  height = 13.5
...
- increase zero-or-one-sized types to two units in included domains (panel set 1)
- (few cases:) increasing zero- or one-sized strata to two units at Flemish level

In groundwater:

```{r}
gw_thresh <- as.numeric(str_match(scenario_name, "sstrunc.*gw([\\d\\.]+)")[,2])
...
```{r}
soil_thresh <- as.numeric(str_match(scenario_name, "sstrunc.*soil([\\d\\.]+)")[,2])
...
# write_to_gpkg_layer(sps_ref, "GW_ref", targetcal = targetcal_ref)
# write_to_gpkg_layer(sps_ref, "GW_NONCELL_ref", types = non_cell_types, targetcal = targetcal_ref)
# write_to_gpkg_layer(sps_ref, "SOIL_ref", "^SOIL", targetcal = targetcal_ref)
