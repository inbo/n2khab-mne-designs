...
# Which types belong to targetpop of schemes_1st_wave and NOT to targetpop of core_schemes_1st_wave?
...
non_core_types <-
  ...
  filter(scheme %in% schemes_1st_wave) %>%
  left_join(
    read_scheme_types() %>%
  ...
  anti_join(
    read_scheme_types(lang = lang) %>%
      select(scheme, type) %>%
      filter(scheme %in% core_schemes_1st_wave) %>%
      distinct(type),
    by = "type"
  ) %>%
  left_join(
