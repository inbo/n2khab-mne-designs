...
# making zones for each type, at resolution 32 meters, to save where centroid intersects type:
for type in "${types[@]}" ; do
while [ `pgrep -c r.mapcalc` -ge 6 ]; do sleep 1; done
       echo "Started zoning for type '${type}' ..."
       r.mapcalc "\"centr_touched_${type}\" = 1 - isnull(\"phab_${type}\")" --overwrite &
done
wait
