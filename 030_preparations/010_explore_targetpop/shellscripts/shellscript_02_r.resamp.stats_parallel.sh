...
for type in "${types[@]}" ; do
   while [ `pgrep -c r.resamp.stats` -gt 4 ]; do sleep 1; done
   echo "Started maximizing for type '${type}' ..."
   r.resamp.stats input=phabs_${type} output=max_${type} method=maximum --overwrite &
done
wait
