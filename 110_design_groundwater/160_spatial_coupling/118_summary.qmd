...
*Distance-rule-dependent site re-use. The higher we set the distance threshold for inclusion of existing sites (x), the more existing watina locations would potentially be re-used for the MNM sampling frame. Black line: actual sampling frame; blue line: regression; grey: bootstrapping interval.*
...
- scales: multi-modal variograms (bootstrapping!), this is plausible
- bias in existing data -> no options
- Wet and Dry Season: not much difference
- including unvalidated data: too tempting to ignore (sample size!!); consistent with filtered data
