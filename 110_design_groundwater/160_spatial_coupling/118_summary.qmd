...
*Distance-rule-dependent site re-use. The higher we set the distance threshold for inclusion of existing sites (x), the more existing watina locations would potentially be re-used for the MNM sampling. Black line: actual sampling; blue dashed line: regression; grey: bootstrapping interval.*
...
- *slope filter* refers to filtering location pairs prior to variogram calculation
- *elevation difference rule* refers to excluding potential re-use locations from MNE because of a certain difference in elevation

I use those consistently in this notebook, but might have confused them elsewhere.
:::


Optionally turning on one or turning off the **slope filter** will affect the shape of the difference-distance curve.
...
- slope **linked to** elevation
  ...
  - soilclass `peat` is intrinsically slope-filtered; however, it keeps rising with `mTAW` (whereas it goes flat with `mMaaiveld`) -> elevation difference is a confounding effect
...
- **bootstrapping:**
  ...
  - indicates *bimodal* outcomes for range parameters
...
- exclusion by **elevation difference rule**
  ...
  - distance-independent for within the $10 m$ radius (practicality: futile to measure $\approx 1 cm$ elevation on short distances)
...
- Wet and Dry Season: not much difference (interest anticipated; will be refined)
...
*Cluster bias. We see it, but it is hard to avoid. At least, the influential clusters are evenly distributed across distances (visual evaluation).*
