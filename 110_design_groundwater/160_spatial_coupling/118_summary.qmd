...
These rules apply equally in cases of self-motivation re-use, as well as hard-limiting the leeway to external stakeholder conciliation.
...
- bias avoidance:
  ...
  - existing sites introduce implicit, unmeasurable bias to the monitoring
...
- common sense framing:
  ...
  - if there is a site fifty meters further, that is obviously not close enough 
...
- external constraints: site accessibility and land owner's interests (conciliation)
...
i.e. a pair of sites $5 m$ apart will be excluded from analysis if they differ more than $0.05 m$ in elevation.
...
Readers interested in the general workings of variogram analysis may refer to [a tutorial](https://github.com/inbo/tutorials/pull/352) written alongside this analysis.
...
given that the measurement sites in question differ by less than $10 cm$ in elevation (via elevation difference rule).
:::
...
To quantify what this threshold would mean for practical roll-out of the monitoring network, we can take a hypothetical ("work in progress") draw of sites ("sample") from the sampling frame (see "Proof Of Concept").
...
In other words, for each site from the hypothetical sample, we calculate the spatial distance in meters to each reference location; this does *not* include pairwise distances within the set of sample locations.
A histogram of "sample sites to watina" cross-distances is one particular realization (one sample) of the global probability distribution of site distances.
...
:::{.callout-warning}
Given the re-use threshold of $10 m$, we anticipate `{r} mean_n_sites` $\left[\right.$ `{r} calculate_n_sites(distance = 10, col = "2.5%")`, `{r} calculate_n_sites(distance = 10, col = "97.5%")` $\left.\right]$ sites to be applicable for inclusion in the MNE data set (`{r} round(100*mean_n_sites / 978, 1)` % of the planned MNE sites).
:::
...
Implementation of a lump $10 cm$ elevation difference would mean that sites with a slope of $>1\%$ could be re-used (e.g. sites $2 m$ apart, $0.08 m$ elevation difference, $4\%$ slope). 
...
Admitting $10 cm$ difference across board also gives field personnell a determined, reasonable margin within which to re-use.
...
Overall water level differences might be of reduced relevance for the long-term projection of a natural system, if that system is determined by the extre time points, rather than by the average.
For example, an extended period of low water levels (drought) might cause critical physical pressure on the vegetation of a protected area.
