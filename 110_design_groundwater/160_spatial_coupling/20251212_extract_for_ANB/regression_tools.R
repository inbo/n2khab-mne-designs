...
print_regression_results <- function(orsl, label = "", indicate_threshold = TRUE) {
  ...
  scale  <- par[1] # related to semivariance
  sigma  <- par[2] # related to actual range; turning point
  nugget <- par[3] # variance at zero distance
  nu     <- par[4] # shape parameter; here: soft-fixed to ~1
  sill   <- scale + nugget

  fmt <- \(num, n = 3) format(num, nsmall = n)

  print(
    glue::glue("{label}: conv {conv}, mse {fmt(eps, 1)}
    at nugget = {fmt(nugget)},
       sill   = {fmt(sill)},
       sigma  = {fmt(sigma)}")
  )
