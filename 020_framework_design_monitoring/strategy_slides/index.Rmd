...
steekproefdata %>% 
    ggplot(aes(x = Maand, y = Locatie)) + 
    geom_tile(data = trendpop, fill = "lightgreen") +
    geom_tile(data = trendpop %>% filter((Maand %% 12) %in% 4:7),
                                        fill = "violet",
                                         alpha = 0.5) +
...
- Doelparameter trend: de *spatiaal gemiddelde* trend (meerjarentrend voor de volledige spatiale doelpopulatie).
...
Verband bij gelijke precisie van de typeschatters en gelijk aandeel van de typeschatters in de groepsschatter:
...
- Design van steekproef: spatiaal of spatiotemporeel?
    - indien continumetingen of een jaarlijkse waarde voor alle locaties beschikbaar is: enkel spatiale steekproef
    - anders: spatiotemporele steekproef
...
- Overgangsfase: gebruik van bestaande meetpunten totdat langere reeksen van aselecte set zijn opgebouwd.
...
## Revisit design {.allowframebreaks}
...
- Louter spatiale steekproeven zijn hier een 'pure panel' (jaarlijkse data van elke locatie)
- Terugkeerperiode?
